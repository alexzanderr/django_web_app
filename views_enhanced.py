...
def _json_response(_func, rest_api, *args, **kwargs):
    _returned = _func(*args, **kwargs)
    ...
    if rest_api:
        return json_api_response(data=_data, status=_status_code)

    return json_response(data=_data, status=_status_code)


def json_response_decorator(_django_view_func):
    @functools.wraps(_django_view_func)
    def inner_wrapper(*args, **kwargs):
        return _json_response(_django_view_func, False, *args, **kwargs)
    return inner_wrapper


def json_api_response_decorator(_django_view_func):
    @functools.wraps(_django_view_func)
    def inner_wrapper(*args, **kwargs):
        return _json_response(_django_view_func, True, *args, **kwargs)
    return inner_wrapper
